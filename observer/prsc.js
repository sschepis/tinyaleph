...
// Box-Muller transform for Gaussian random numbers (browser-compatible)
// ============================================================================
...
// ============================================================================
// DISCRETE PHASE DYNAMICS (from discrete.pdf)
// Integer Sine Tables for discretized oscillator evolution
// ============================================================================

/**
 * Integer sine table for discrete phase dynamics
 * From discrete.pdf: Uses M-point discretization for deterministic coupling
 *
 * INT_SINE_TABLE[M] contains precomputed sin(2πk/M) * SCALE_FACTOR for k ∈ [0, M-1]
 * This enables tick-only updates without floating-point trigonometry.
 */
const INT_SINE_M = 256; // Discretization resolution (power of 2 for fast modulo)
const INT_SINE_SCALE = 10000; // Scale factor for integer arithmetic

const INT_SINE_TABLE = new Int32Array(INT_SINE_M);
for (let k = 0; k < INT_SINE_M; k++) {
    INT_SINE_TABLE[k] = Math.round(Math.sin(2 * Math.PI * k / INT_SINE_M) * INT_SINE_SCALE);
}

/**
 * Fast integer sine lookup
 * @param {number} phaseIndex - Phase index in [0, M-1]
 * @returns {number} Scaled sine value
 */
function intSin(phaseIndex) {
    return INT_SINE_TABLE[(phaseIndex & (INT_SINE_M - 1))];
}

/**
 * Convert continuous phase to discrete index
 * @param {number} phase - Phase in radians [0, 2π]
 * @returns {number} Phase index in [0, M-1]
 */
function phaseToIndex(phase) {
    // Normalize to [0, 1] then scale to [0, M-1]
    const normalized = ((phase % (2 * Math.PI)) + 2 * Math.PI) % (2 * Math.PI);
    return Math.round(normalized * INT_SINE_M / (2 * Math.PI)) & (INT_SINE_M - 1);
}

/**
 * Convert discrete index back to continuous phase
 * @param {number} index - Phase index in [0, M-1]
 * @returns {number} Phase in radians [0, 2π]
 */
function indexToPhase(index) {
    return (index & (INT_SINE_M - 1)) * 2 * Math.PI / INT_SINE_M;
}

// ============================================================================
// HISTOGRAM COHERENCE (from discrete.pdf Section 4.1)
// C_bin(t) = max_k(b_k(t)) / |P|
// ============================================================================

/**
 * Compute histogram coherence from phase distribution
 * From discrete.pdf: Measures how concentrated phases are in bins
 *
 * C_bin(t) = max_k(b_k(t)) / |P|
 * where b_k(t) is the count of oscillators in bin k
 *
 * Higher values indicate more phase coherence (oscillators in same bin).
 *
 * @param {Array<number>} phases - Array of phases in radians
 * @param {number} numBins - Number of histogram bins (default: 16)
 * @returns {Object} Histogram coherence metrics
 */
function computeHistogramCoherence(phases, numBins = 16) {
    const binCounts = new Int32Array(numBins);
    const binSize = 2 * Math.PI / numBins;
    
    // Count oscillators in each bin
    for (const phase of phases) {
        const normalized = ((phase % (2 * Math.PI)) + 2 * Math.PI) % (2 * Math.PI);
        const binIndex = Math.min(numBins - 1, Math.floor(normalized / binSize));
        binCounts[binIndex]++;
    }
    ...
    for (let k = 0; k < numBins; k++) {
        if (binCounts[k] > maxCount) {
            maxCount = binCounts[k];
            maxBin = k;
        }
    }
    
    // Histogram coherence: C_bin = max_k(b_k) / |P|
    const N = phases.length;
    const coherence = N > 0 ? maxCount / N : 0;
    
    // Compute entropy of the distribution
    let entropy = 0;
    for (let k = 0; k < numBins; k++) {
        if (binCounts[k] > 0) {
            const p = binCounts[k] / N;
    ...
    return {
        coherence,          // C_bin(t) ∈ [0, 1]
        maxCount,           // Maximum bin count
        maxBin,             // Index of maximum bin
        maxBinPhase: maxBin * binSize + binSize / 2, // Center of max bin
        entropy,            // Distribution entropy
        meanPhase,          // Circular mean phase
        binCounts: Array.from(binCounts),
        numBins,
        numPhases: N
    };
}
...
class PRSCLayer {
    ...
    // ════════════════════════════════════════════════════════════════════
    // DISCRETE PHASE DYNAMICS (from discrete.pdf)
    // Integer sine tables and histogram coherence
    // ════════════════════════════════════════════════════════════════════
    
    /**
     * Discrete Kuramoto coupling using integer sine table
     * From discrete.pdf: Enables tick-only deterministic updates
     *
     * @param {PrimeOscillator} osc - Target oscillator
     * @returns {number} Coupling contribution (scaled integer)
     */
    discreteKuramotoCoupling(osc) {
        const targetIdx = phaseToIndex(osc.phase);
        let couplingSum = 0;
        const N = this.oscillators.length;
        
        for (const other of this.oscillators) {
            if (other !== osc) {
                const otherIdx = phaseToIndex(other.phase);
                // sin(φ_other - φ_target) using addition formula with table lookup
                // sin(a-b) = sin(a)cos(b) - cos(a)sin(b)
                // Approximate by direct difference modulo M
                const phaseDiff = ((otherIdx - targetIdx) + INT_SINE_M) & (INT_SINE_M - 1);
                const sinDiff = intSin(phaseDiff);
                
                // Weight by amplitude
                const weight = Math.round(Math.min(1, other.amplitude + 0.1) * 1000);
                couplingSum += (sinDiff * weight) / 1000;
            }
        }
        
        // Scale by coupling constant and N
        return (this.K * couplingSum) / (N * INT_SINE_SCALE);
    }
    
    /**
     * Tick with discrete phase dynamics
     * Uses integer sine table for deterministic evolution
     *
     * @param {number} dt - Time step (optional)
     * @returns {Object} Tick metrics including histogram coherence
     */
    discreteTick(dt = null) {
        dt = dt || this.dt;
        
        // Compute discrete couplings
        const couplings = this.oscillators.map(osc => this.discreteKuramotoCoupling(osc));
        
        // Update phases and amplitudes
        for (let i = 0; i < this.oscillators.length; i++) {
            const osc = this.oscillators[i];
            
            // Phase evolution with integer coupling
            osc.phase += 2 * Math.PI * osc.frequency * dt * this.speed;
            osc.phase += couplings[i] * dt;
        ...
        const histogramMetrics = computeHistogramCoherence(phases, 16);
        
        // Standard coherence for comparison
        const globalCoherence = this.globalCoherence();
        const orderParam = this.orderParameter();
        ...
        this.coherenceHistory.push({
            ...
            histogramCoherence: histogramMetrics.coherence,
            orderParameter: orderParam,
            activeCount: this.activeCount(),
            maxBin: histogramMetrics.maxBin
        });
    ...
    /**
     * Get histogram coherence for current state
     * @param {number} numBins - Number of bins
     * @returns {Object} Histogram coherence metrics
     */
    getHistogramCoherence(numBins = 16) {
        const phases = this.oscillators.map(o => o.phase);
        return computeHistogramCoherence(phases, numBins);
    }
    
    /**
     * Get discrete state information
     * @returns {Object} Discrete dynamics state
     */
    getDiscreteState() {
        const phases = this.oscillators.map(o => o.phase);
        const histogram = computeHistogramCoherence(phases, 16);
        
        return {
            discretizationM: INT_SINE_M,
            scaleFactorS: INT_SINE_SCALE,
            histogramCoherence: histogram.coherence,
            histogramEntropy: histogram.entropy,
            maxBin: histogram.maxBin,
            maxBinPhase: histogram.maxBinPhase,
            binDistribution: histogram.binCounts,
